  ...
  if (statusModal) {
    statusTriggerButtons.forEach((btn) => {
      ...
      btn.addEventListener("click", (e) => {
        ...
        if (recruitmentType)
          statusModal.dataset.recruitmentType = recruitmentType;
...
const modalEmploymentTypeValue = document.getElementById(
  "modalEmploymentTypeValue"
);
const modalWorkScheduleBadge = document.getElementById("modalWorkSchedule");
const modalWorkScheduleValue = document.getElementById(
  "modalWorkScheduleValue"
);
...
function formatSalaryRange(min, max) {
  try {
    ...
    if (hasMin && hasMax) {
      return `${formatter.format(numericMin)} - ${formatter.format(
        numericMax
      )}`;
    }
...
function toggleJobActionButtons(enable, suppressMessage = false) {
  ...
  if (!enable && jobActionFeedback && !suppressMessage) {
    jobActionFeedback.textContent =
      "Open a reported job to enable moderation actions.";
...
function resetJobActionFeedback() {
  ...
  jobActionFeedback.textContent = "";
  jobActionFeedback.classList.remove(
    "text-success",
    "text-danger",
    "text-muted"
  );
...
function showJobActionFeedback(message, type = "success") {
  ...
  jobActionFeedback.classList.add(
...
async function handleJobAction(action) {
  ...
  try {
    ...
    if (modalJobStatus)
      modalJobStatus.innerText =
        formatStatusLabel(data.job_status) || "Updated";
...
const applicantActionButtons = document.querySelectorAll(
...
async function handleApplicantReportAction(button) {
  ...
  try {
    ...
    updateApplicantReportRow(
      reportId,
      data.report_status || formatStatusLabel(action)
    );
...
function updateApplicantReportRow(reportId, status) {
  const row = document.querySelector(
    `tr[data-applicant-report-id="${reportId}"]`
  );
  if (!row) return;
